...
|---|---|---|
| Bun suite | PASS | 159 tests, zero failures; includes real controller against fake OBS, invalid requests, lifecycle, file/ACL and launcher failures. |
...
Session-area cleanup: one state-dependent primary action, compact guidance and confirmed source-mute summary. Native stopped/enabled layouts checked with both sources muted; original enabled-but-muted state and levels restored. Tests cover unconfirmed-state Stop visibility and source-state summaries.
